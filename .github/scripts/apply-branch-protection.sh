...
# .github/rulesets/*.json. Idempotent: re-running updates the existing ruleset
...
# Preflight: repository rulesets require either a public repository or a paid
# plan (GitHub Pro and above for private repos). On a private repo on the Free
# plan the rulesets API returns HTTP 403, so the apply would fail mid-run with a
# raw error. Probe once up front and turn that into a clear, actionable message.
...
error: repository rulesets cannot be applied to '$REPO' on its current plan.
  ...
  Until then the CI/SAST workflows still RUN on pushes and PRs into main and
  release/* branches, but nothing BLOCKS a merge when they fail. Re-run this
  script once the repo is public or upgraded; it is idempotent.
EOF
    exit 2
  fi
