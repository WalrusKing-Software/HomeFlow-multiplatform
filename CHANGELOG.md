...
- **Cancel button on the server-connect screen.** While the app is checking
  whether a self-hosted server is reachable, the "Connect" button becomes a
  "Cancel" button — tap it to stop the in-flight check instead of waiting it out,
  returning to the hostname entry screen.
