...
import org.homeflow.core.domain.MIN_CYCLES_FOR_STATS
...
class LocalAnalyticsTest {
    ...
    /** Creates a closed cycle with daily logs on each day having a blood_flow entry. */
    ...
    @Test
    fun `cycleStats returns nulls with zero closed cycles`() = runTest {
        val stats = (ds().getCycleStats() as ApiResult.Success).value
        assertNull(stats.averageCycleLength)
    }

    @Test
    fun `cycleStats returns nulls with one closed cycle (below threshold)`() = runTest {
        val ds = ds()
        ds.createClosedCycleWithFlow("2024-01-15", "2024-02-11", listOf("2024-01-15", "2024-01-16", "2024-01-17", "2024-01-18", "2024-01-19"))
        val stats = (ds.getCycleStats() as ApiResult.Success).value
        assertNull(stats.averageCycleLength)
    }

    @Test
    fun `cycleStats computes average with two closed cycles`() = runTest {
        val ds = ds()
        // Cycle 1: 28 days (Jan 15 → Feb 11 inclusive)
        ds.createClosedCycleWithFlow(
            "2024-01-15", "2024-02-11",
            listOf("2024-01-15", "2024-01-16", "2024-01-17", "2024-01-18", "2024-01-19"),
        )
        // Cycle 2: 30 days (Feb 12 → Mar 12 inclusive)
        ds.createClosedCycleWithFlow(
    ...
    @Test
    fun `periodLengthChart returns one point per closed cycle oldest first`() = runTest {
        val ds = ds()
        ds.createClosedCycleWithFlow(
            "2024-01-15", "2024-02-11",
    ...
    @Test
    fun `getOvulationPrediction returns null predictions with insufficient data`() = runTest {
    ...
    @Test
    fun `getOvulationPrediction returns 3 predictions with sufficient data`() = runTest {
        val ds = ds()
    ...
    @Test
    fun `getSleepPredictions always returns success`() = runTest {
        val result = ds().getSleepPredictions()
        assertIs<ApiResult.Success<*>>(result)
    }

    @Test
    fun `getSleepPredictions returns null phases with insufficient data`() = runTest {
