...
class LocalDataSourceParityTest {
    ...
    @Test
    fun `empty category fields are null not empty list in daily log`() =
        runTest {
            val ds = ds()
    ...
    @Test
    fun `analytics getters always return Success even with no data`() =
        runTest {
            val ds = ds()
    ...
    @Test
    fun `getCycleStats returns all nulls with fewer than 2 closed cycles`() =
        runTest {
            val ds = ds()
    ...
    @Test
    fun `getCycles returns newest cycle first`() =
        runTest {
            val ds = ds()
    ...
    @Test
    fun `putOptionIds replacing options removes previous selection`() =
        runTest {
            val ds = ds()
            val cycle = (ds.createCycle("2024-01-15") as ApiResult.Success).value
            ds.createDailyLog("2024-01-15", cycle.id)
    ...
    @Test
    fun `sex payload stored as option ids and read back correctly`() =
        runTest {
            val ds = ds()
            val cycle = (ds.createCycle("2024-01-15") as ApiResult.Success).value
            ds.createDailyLog("2024-01-15", cycle.id)
    ...
    @Test
    fun `putOptionIds with empty list clears the selection`() =
        runTest {
            val ds = ds()
            val cycle = (ds.createCycle("2024-01-15") as ApiResult.Success).value
            ds.createDailyLog("2024-01-15", cycle.id)
    ...
    @Test
    fun `daily log id and cycleId survive round-trip`() =
        runTest {
            val ds = ds()
            val cycle = (ds.createCycle("2024-01-15") as ApiResult.Success).value
            ds.createDailyLog("2024-01-15", cycle.id)
    ...
    @Test
    fun `closeCycle on already-closed cycle updates end date`() =
        runTest {
            val ds = ds()
            val cycle = (ds.createCycle("2024-01-15") as ApiResult.Success).value
