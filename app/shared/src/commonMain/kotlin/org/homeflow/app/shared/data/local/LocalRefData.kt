 ...
 */
class LocalRefData(private val db: HomeFlowDb) {
    ...
    fun categoryIdBySlug(slug: String): String? =
        db.refDataQueries.selectCategoryBySlug(slug).executeAsOneOrNull()?.id

    /** Returns the set of valid option ids for a given category id. */
    fun optionIdsForCategory(categoryId: String): Set<String> =
        db.refDataQueries.selectOptionsByCategory(categoryId)
            .executeAsList()
            .map { it.id }
            .toSet()
    ...
    fun allCategorySlugs(): Set<String> =
        db.refDataQueries.selectAllCategories()
            .executeAsList()
            .map { it.slug }
            .toSet()
