...
/** Settings: reorder the dashboard tracking categories with up/down controls and persist them. */
@Composable
fun PreferencesScreen(repository: HomeFlowRepository) {
            ...
            onClick = {
                scope.launch {
                    ...
                    when (val result = repository.savePreferences(order.map { it.slug })) {
                        ...
                        is ApiResult.Failure -> status = result.message
                    }
                    busy = false
                }
            },
